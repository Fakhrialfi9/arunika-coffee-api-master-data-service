...
section() { printf '\n=== %s ===\n' "$1"; }
run_required() { local label="$1"; shift; printf '  [RUN ] %s\n' "$label"; if "$@"; then pass "$label"; else fail "$label"; fi; }
file_nonempty() { [[ -s "$1" ]]; }
command_exists() { command -v "$1" >/dev/null 2>&1; }

# Load local verification configuration when the caller has not exported it.
...
MYSQL_ARGS=()
[[ -n "${DATABASE_HOST:-}" ]] && MYSQL_ARGS+=(--host="$DATABASE_HOST")
[[ -n "${DATABASE_PORT:-}" ]] && MYSQL_ARGS+=(--port="$DATABASE_PORT")
[[ -n "${DATABASE_USER:-}" ]] && MYSQL_ARGS+=(--user="$DATABASE_USER")
[[ -n "${DATABASE_PASSWORD:-}" ]] && MYSQL_ARGS+=(--password="$DATABASE_PASSWORD")
[[ -n "${DATABASE_NAME:-}" ]] && MYSQL_ARGS+=("$DATABASE_NAME")
...
if [[ -n "${DATABASE_NAME:-}" && -n "${DATABASE_USER:-}" ]] && command_exists mysql; then
  constraint_sql="SELECT CONCAT('PK|', TABLE_NAME, '|', GROUP_CONCAT(COLUMN_NAME ORDER BY ORDINAL_POSITION)) FROM information_schema.KEY_COLUMN_USAGE WHERE CONSTRAINT_SCHEMA=DATABASE() AND CONSTRAINT_NAME='PRIMARY' GROUP BY TABLE_NAME UNION ALL SELECT CONCAT('UNIQUE|', TABLE_NAME, '|', CONSTRAINT_NAME, '|', GROUP_CONCAT(COLUMN_NAME ORDER BY ORDINAL_POSITION)) FROM information_schema.KEY_COLUMN_USAGE WHERE CONSTRAINT_SCHEMA=DATABASE() AND CONSTRAINT_NAME <> 'PRIMARY' AND CONSTRAINT_NAME IN (SELECT CONSTRAINT_NAME FROM information_schema.TABLE_CONSTRAINTS WHERE CONSTRAINT_SCHEMA=DATABASE() AND CONSTRAINT_TYPE='UNIQUE') GROUP BY TABLE_NAME, CONSTRAINT_NAME UNION ALL SELECT CONCAT('FK|', TABLE_NAME, '|', CONSTRAINT_NAME, '|', COLUMN_NAME, '|', REFERENCED_TABLE_NAME, '|', REFERENCED_COLUMN_NAME) FROM information_schema.KEY_COLUMN_USAGE WHERE CONSTRAINT_SCHEMA=DATABASE() AND REFERENCED_TABLE_NAME IS NOT NULL ORDER BY 1;"
...
if [[ -f prisma.config.ts ]] && grep -Eq "seed:[[:space:]]*['\"]node --experimental-strip-types prisma/seed\.ts['\"]" prisma.config.ts && [[ -f prisma/seed.ts ]]; then
  pass "Prisma seed entry point is configured and exists"
elif [[ -f prisma.config.ts ]] && [[ -f prisma/seed.ts ]]; then
  fail "Prisma seed entry point is not configured with the supported deterministic runner"
else
...
if [[ "${VERIFY_ALLOW_SEED_MUTATION:-false}" == "true" ]]; then
  if [[ "${DATABASE_NAME:-}" == *_test ]]; then
    ...
    if NODE_ENV=test npm run prisma:seed && NODE_ENV=test npm run prisma:seed; then pass "Seed repeatability/idempotency"; else fail "Seed repeatability/idempotency"; fi
  else
    fail "Seed mutation requested but DATABASE_NAME must end with _test"
  fi
else
...
if [[ -n "${DATABASE_NAME:-}" && -n "${DATABASE_USER:-}" ]] && command_exists mysql; then
  ...
  # Only flag truly redundant non-unique indexes: a non-unique index whose leading
  # column is already the leading column of another non-unique index. A single-column
  # FK index plus a UNIQUE composite index is legitimate and must not be reported.
  duplicate_index_sql="SELECT s.TABLE_NAME, s.COLUMN_NAME, COUNT(DISTINCT s.INDEX_NAME) AS non_unique_leading_indexes FROM information_schema.STATISTICS s JOIN information_schema.STATISTICS l ON l.TABLE_SCHEMA=s.TABLE_SCHEMA AND l.TABLE_NAME=s.TABLE_NAME AND l.COLUMN_NAME=s.COLUMN_NAME AND l.SEQ_IN_INDEX=1 AND l.NON_UNIQUE=1 WHERE s.TABLE_SCHEMA=DATABASE() AND s.SEQ_IN_INDEX=1 AND s.NON_UNIQUE=1 GROUP BY s.TABLE_NAME, s.COLUMN_NAME HAVING COUNT(DISTINCT s.INDEX_NAME) > 1 ORDER BY s.TABLE_NAME, s.COLUMN_NAME;"
  duplicate_indexes="$(mysql_query "$duplicate_index_sql" 2>/dev/null || true)"
  if [[ -z "$duplicate_indexes" ]]; then pass "No redundant non-unique leading-column indexes"; else printf '%s\n' "$duplicate_indexes"; fail "Potential duplicate/redundant non-unique indexes require review"; fi
else
