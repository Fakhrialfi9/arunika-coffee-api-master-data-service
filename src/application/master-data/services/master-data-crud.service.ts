...
const SORT_FIELDS = new Set(['id', 'uuid', 'code', 'name', 'sortOrder', 'createdAt', 'updatedAt', 'year']);
...
export class MasterDataCrudService {
  ...
  async list(entity: MasterDataEntityName, query: MasterDataListQuery = {}) {
    ...
    if (!Number.isInteger(page) || page < 1) throw new MasterDataValidationError('page must be an integer greater than 0');
    if (!Number.isInteger(limit) || limit < 1 || limit > 100) {
      throw new MasterDataValidationError('limit must be an integer between 1 and 100');
    }
    ...
    if (query.sortBy !== undefined && !SORT_FIELDS.has(query.sortBy)) {
      throw new MasterDataValidationError(`Unsupported master-data sort field: ${query.sortBy}`);
    }
    for (const field of Object.keys(query.filters ?? {})) {
      if (!FILTER_FIELDS.has(field)) throw new MasterDataValidationError(`Unsupported master-data filter: ${field}`);
    }
  ...
  private async validateRelations(entity: MasterDataEntityName, data: MasterDataWrite): Promise<void> {
    const exists = async (target: MasterDataEntityName, id: unknown, field: string, required = false) => {
      if (id === undefined || id === null || id === '') {
        if (required) throw new MasterDataValidationError(`${entity}.${field} is required`);
        return null;
      }
      const normalizedId = this.toIdentifier(id);
      if (!normalizedId) throw new MasterDataValidationError(`${entity}.${field} must be a valid identifier`);
    ...
    if (entity === 'farmer') {
      ...
      if (organization && region && String(organization.regionId) !== String(region.id)) {
        throw new MasterDataValidationError('Farmer organizationId must belong to regionId');
      }
    }
    ...
    if (entity === 'sensoryProfile') await exists('coffeeBean', data.coffeeBeanId, 'coffeeBeanId', true);
    if (entity === 'sensoryProfileFlavor') {
      await exists('sensoryProfile', data.sensoryProfileId, 'sensoryProfileId', true);
      await exists('flavorProfile', data.flavorProfileId, 'flavorProfileId', true);
      if (data.sortOrder !== undefined && (!Number.isInteger(data.sortOrder) || Number(data.sortOrder) < 0)) {
        throw new MasterDataValidationError('SensoryProfileFlavor.sortOrder must be a non-negative integer');
      }
    }
    ...
    if (entity === 'coffeeBean') {
      ...
      if (variety && String(variety.speciesId) !== String(species?.id)) {
        throw new MasterDataValidationError('CoffeeBean varietyId must belong to the selected speciesId');
      }
      if (farm && farmer && String(farm.farmerId) !== String(farmer.id)) {
        throw new MasterDataValidationError('CoffeeBean farmId must belong to farmerId');
      }
      if (farmer && region && String(farmer.regionId) !== String(region.id)) {
        throw new MasterDataValidationError('CoffeeBean farmerId must belong to regionId');
      }
    ...
    if (entity === 'farmer') this.validateYear(data.farmingSinceYear, 'farmingSinceYear');
    if (entity === 'organization') this.validateYear(data.establishedYear, 'establishedYear');
  }

  private validateCoffeeBeanNumbers(data: MasterDataWrite): void {
    this.nonNegative(data.cuppingScore, 'cuppingScore');
    if (data.cuppingScore !== undefined && Number(data.cuppingScore) > 100) throw new MasterDataValidationError('cuppingScore must be between 0 and 100');
    this.nonNegative(data.moisture, 'moisture');
    if (data.moisture !== undefined && Number(data.moisture) > 100) throw new MasterDataValidationError('moisture must be between 0 and 100');
  ...
  private validateFarmNumbers(data: MasterDataWrite): void {
    ...
    if (data.altitudeMin !== undefined && data.altitudeMax !== undefined && Number(data.altitudeMin) > Number(data.altitudeMax)) {
      throw new MasterDataValidationError('altitudeMin must not exceed altitudeMax');
    }
    if (data.latitude !== undefined && (Number(data.latitude) < -90 || Number(data.latitude) > 90)) throw new MasterDataValidationError('latitude must be between -90 and 90');
    if (data.longitude !== undefined && (Number(data.longitude) < -180 || Number(data.longitude) > 180)) throw new MasterDataValidationError('longitude must be between -180 and 180');
  }
  ...
  private validateHarvestSeason(data: MasterDataWrite): void {
    ...
    for (const field of ['startMonth', 'endMonth']) {
      ...
      if (value !== undefined && value !== null && (!Number.isInteger(value) || Number(value) < 1 || Number(value) > 12)) {
        throw new MasterDataValidationError(`${field} must be an integer between 1 and 12`);
      }
    }
  }
  ...
  private validateYear(value: unknown, field: string): void {
    ...
    if (!Number.isInteger(value) || Number(value) < 1 || Number(value) > 9999) throw new MasterDataValidationError(`${field} must be a valid year`);
  }

  private nonNegative(value: unknown, field: string): void {
    if (value === undefined || value === null) return;
    if (typeof value !== 'number' || !Number.isFinite(value) || value < 0) throw new MasterDataValidationError(`${field} must be a non-negative number`);
  }
